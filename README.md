# waybar-themes for Omarchy

## Screenshots V1

- first creation of my own Waybar config <br>

  ![image](https://github.com/kxxian/waybar-themes/blob/main/screenshots/v1-ss-2.png?raw=true)

## Screenshots V2

- inspired to solarized osaka colors <br>

  ![image](https://github.com/kxxian/waybar-themes/blob/main/screenshots/v2-ss-2.png?raw=true)

## Screenshots V3

- added network bandwith up/down, backlight, moved mpris in left module, and changed clock format <br>

  ![image](https://github.com/kxxian/waybar-themes/blob/main/screenshots/v3-ss.png?raw=true)

## Screenshots V4

- credits to [HANCORE-linux](https://github.com/HANCORE-linux), I only tweaked his config a little bit for my own taste. I used Mecha no Onna [V2] (メカの女) theme and for waybar I used 6.ba

  ![image](https://github.com/kxxian/waybar-themes/blob/main/screenshots/v4-ss.png?raw=true)
